...
function LoginCard() {
  ...
  const [email, setEmail] = useState(demoSent ? copy.auth.login.emailPlaceholder : "")
  ...
  const submit = async () => {
    ...
    setBusy(false)
    if (!result.sent) return setError(result.message ?? copy.auth.genericError)
    setStep("sent")
    startCooldown()
  }
  ...
  const resend = async () => {
    ...
    if (result.rateLimited) setSentNote(result.message ?? copy.auth.rateLimited)
  }
        ...
        <div style={{ flex: "none", padding: "26px 0 4px" }}>
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ fontSize: 15, color: "var(--txt-2)", lineHeight: 1.6, margin: "0 0 24px", textAlign: "center" }}>
              {copy.auth.login.sentIntro()[0]}<span style={{ color: "var(--txt)", fontWeight: "var(--w-semi)" as never }}>{email}</span>{copy.auth.login.sentIntro()[1]}
            </p>
            ...
            <div style={{ background: "var(--shell)", borderRadius: "var(--r2)", padding: 22, marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, paddingBottom: 14, borderBottom: "1px solid var(--line)" }}>
                <span style={{ fontFamily: "var(--mono)", fontSize: 11, letterSpacing: ".11em", textTransform: "uppercase", color: "var(--txt-3)", flex: 1 }}>{copy.auth.login.nothingArrived}</span>
                <span
                  onClick={resend}
                ...
                >
                  {cooldown ? copy.auth.login.resendIn(cooldown) : copy.auth.login.resend}
                </span>
              </div>
              {sentNote && (
              ...
              <ul style={{ margin: "14px 0 0", paddingLeft: 18, fontSize: 13.5, color: "var(--txt-2)", lineHeight: 1.75 }}>
                <li>{copy.auth.login.tipSpam} <span style={{ fontFamily: "var(--mono)", fontSize: 12.5 }}>{copy.auth.login.tipSender}</span></li>
                <li>{copy.auth.login.tipFilters}</li>
                <li>{copy.auth.login.tipDevice}</li>
              </ul>
            </div>
      ...
      <div style={{ flex: "none", padding: "26px 0 4px" }}>
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{ fontSize: 15, color: "var(--txt-2)", lineHeight: 1.6, margin: "0 0 28px", textAlign: "center" }}>
            {copy.auth.login.intro}
          </p>

          <label style={{ display: "block", marginBottom: 14 }}>
            <span style={{ display: "block", fontFamily: "var(--mono)", fontSize: 11, letterSpacing: ".11em", textTransform: "uppercase", color: "var(--txt-3)", marginBottom: 9 }}>{copy.auth.login.emailLabel}</span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, margin: "30px 0 22px" }}>
            ...
            <span style={{ fontFamily: "var(--mono)", fontSize: 10.5, letterSpacing: ".11em", textTransform: "uppercase", color: "var(--txt-4)" }}>{copy.auth.login.or}</span>
            <span style={{ height: 1, background: "var(--line)", flex: 1, display: "block" }} />
          </div>
          <span className="sh-pick" title={copy.auth.login.ssoUnavailable} style={{ display: "flex", width: "100%", boxSizing: "border-box", alignItems: "center", justifyContent: "center", gap: 10, border: "1px solid var(--line-2)", borderRadius: 999, padding: "14px 20px", fontSize: 14.5, fontWeight: "var(--w-semi)" as never, color: "var(--txt-2)", cursor: "pointer" }}>
          ...
          <p style={{ fontSize: 12.5, color: "var(--txt-3)", lineHeight: 1.6, margin: "26px 0 0" }}>
            {copy.auth.login.terms()[0]}<a href="#" style={{ textDecorationThickness: 1 }}>{copy.auth.login.termsWord}</a>{copy.auth.login.terms()[1]}<a href="#" style={{ textDecorationThickness: 1 }}>{copy.auth.login.privacyWord}</a>{copy.auth.login.terms()[2]}
          </p>
        </div>
      </div>
    </>
