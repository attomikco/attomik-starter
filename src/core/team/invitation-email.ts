 ...
 * (Starter Emails / README §6): one job, literal hex resolved from the
 * base skin (mail clients strip custom properties), 620px single column,
...
export function invitationEmail(input: InvitationEmailInput): {
  ...
  const { workspaceName, inviterEmail, role, acceptUrl, expiresInDays } = input
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#e7e8eb;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="620" cellpadding="0" cellspacing="0" style="max-width:620px;width:100%;background:#ffffff;border-radius:16px;padding:40px;">
        <tr><td style="font-family:ui-monospace,monospace;font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:#7b8188;padding-bottom:22px;">${escapeHtml(workspaceName)}</td></tr>
        ...
        <tr><td style="padding-bottom:22px;">
          <a href="${acceptUrl}" style="display:inline-block;background:oklch(0.52 0.160 250);color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;border-radius:999px;padding:13px 26px;">Accept invitation</a>
        </td></tr>
