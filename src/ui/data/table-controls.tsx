...
/** Status tabs on the shell pill, from the reference table toolbar — the shared TabStrip (also used by RecordLayout's tabs prop). */
export function StatusTabs({
  ...
  return <TabStrip tabs={tabs} active={active} onPick={onPick} />
}
...
export function ColumnPicker<T>({
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      />
      {open && (
        <div style={{ position: "absolute", top: 46, right: 0, zIndex: 40, width: 236, maxWidth: "calc(100vw - 24px)", background: "var(--card)", border: "1px solid var(--line)", borderRadius: "var(--r2)", boxShadow: "0 18px 40px rgba(0,0,0,.14)", padding: 12, animation: "sh-rise .14s ease-out" }}>
...
export function SearchInput({
  ...
  return (
    // flex-basis 200px replaces a hard minWidth: 200 — same ~200px starting
    // size when there's room (flex-grow stays 0, unchanged, so it doesn't
    // expand into spare row space any differently than before), but
    // minWidth: 0 lets it actually shrink to fit whatever width its row has
    // left instead of forcing the row past it on a narrow phone.
    <span className="ui-field" style={{ display: "flex", alignItems: "center", gap: 11, background: "var(--shell)", borderRadius: "var(--r3)", padding: "9px 14px", flex: "0 1 200px", minWidth: 0, maxWidth: "100%", boxSizing: "border-box" }}>
