...
export function SaveBar({
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", flexWrap: "wrap", gap: "10px 12px", flex: "none", borderTop: "1px solid var(--line)", paddingTop: 16 }}>
      <span role={state === "error" ? "alert" : undefined}
        style={{ flex: "1 1 200px", minWidth: 0, fontFamily: "var(--mono)", fontSize: 11, color: state === "error" ? "var(--bad)" : state === "dirty" ? "var(--warn)" : "var(--txt-4)" }}>
        {hintText}
      </span>
      {onDiscard && (
